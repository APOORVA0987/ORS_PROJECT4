...
public class CollegeCtl extends BaseCtl {
	...
	protected boolean validate(HttpServletRequest request) {
		...
		if (DataValidator.isNull(request.getParameter("name"))) {
			...
			request.setAttribute("name", "First Name should contain alphabets only");
			pass = false;
		}
		...
		if (DataValidator.isNull(request.getParameter("state"))) {
			...
			request.setAttribute("state", "State Name contain alphabets only");
			pass = false;
		}
		if (DataValidator.isNull(request.getParameter("city"))) {
			...
			request.setAttribute("city", "City Name contain alphabets only");
			pass = false;
		} else if (!DataValidator.isName(request.getParameter("city"))) {
			request.setAttribute("city", "City Name contain alphabets only");
			pass = false;
		...
		if (DataValidator.isNull(request.getParameter("phoneNo"))) {
			...
			request.setAttribute("phoneNo", "Mobile No. must be 10 Digit and No. Series starts with 6-9");
			pass = false;
		}
