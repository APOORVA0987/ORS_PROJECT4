...
public class MarksheetCtl extends BaseCtl {
	...
	protected boolean validate(HttpServletRequest request) {
		...
		if (DataValidator.isNull(request.getParameter("physics"))
			...
			request.setAttribute("physics", "Marks cannot less be then 0 ");
			pass = false;
		} else if (DataUtility.getInt(request.getParameter("physics")) > 100) {
			request.setAttribute("physics", "Marks cannot be more then 100");
			pass = false;
		...
		if (DataValidator.isNull(request.getParameter("chemistry"))) {
			...
			request.setAttribute("chemistry", "Marks cannot More then 100");
			pass = false;

		} else if (DataUtility.getInt(request.getParameter("chemistry")) < 0) {
			request.setAttribute("chemistry", "Marks cannot less then 0 ");
			pass = false;
		...
		if (DataValidator.isNull(request.getParameter("maths"))) {
			...
			request.setAttribute("maths", "Marks cannot be more then 100");
			pass = false;

		} else if (DataUtility.getInt(request.getParameter("maths")) < 0) {
			request.setAttribute("maths", "Marks cannot be less then 0 ");
			pass = false;
